...
Meteor.publish('trackingViewPub', function(studentId, weekId) {
	...
	return [
		...
		SchoolWork.find({_id: {$in: schoolWorkIds}, groupId: groupId, studentId: studentId, deletedOn: { $exists: false }}, {sort: {name: 1}, fields: {order: 1, name: 1, studentId: 1, schoolYearId: 1}}),
		// Resources.find({groupId: groupId, deletedOn: { $exists: false }, _id: {$in: resourceIds}}, {sort: {title: 1}, fields: {title: 1, type: 1, link: 1}}),
		Lessons.find({groupId: groupId, deletedOn: { $exists: false }, schoolWorkId: {$in: lessonSchoolWorkIds}, weekId: weekId}, {sort: {order: 1}, fields: {order: 1, assigned: 1, completed: 1, schoolWorkId: 1, weekId: 1}})
	]
});
