	...
	addTestData() {
		...
		let fixtureStudents = [
			{insertOne: {"document": {
				...
				birthday: moment('Dec 3, 2001').format('YYYY-MM-DD'),
				groupId: groupId, 
				userId: userId, 
				createdOn: new Date()
			...
			{insertOne: {"document": {
				...
				birthday: moment('Jun 2, 2004').format('YYYY-MM-DD'),
				groupId: groupId, 
				userId: userId, 
				createdOn: new Date()
			}}},
			{insertOne: {"document": {
				...
				birthday: moment('Apr 13, 2006').format('YYYY-MM-DD'),
				groupId: groupId, 
				userId: userId, 
				createdOn: new Date()
			}}},
			{insertOne: {"document": {
				...
				birthday: moment('Jun 6, 2010').format('YYYY-MM-DD'),
				groupId: groupId, 
				userId: userId, 
				createdOn: new Date()
			}}},
		...
		let createdStudentIds = null;
		let createdSchoolYearIds = null;
		let createdTermIds = null;
		let createdWeekIds = null;
		let createdResourceIds = null;
		let createdSchoolWorkIds = null;
		let createdLessonIds = null;

		Students.rawCollection().bulkWrite(
			fixtureStudents
		).then((students) => {
			createdStudentIds = _.values(students.insertedIds);
			return SchoolYears.rawCollection().bulkWrite(fixtureSchoolYears)
		}).then((schoolYears) => {
			let schoolYearIds = _.values(schoolYears.insertedIds);
			createdSchoolYearIds = schoolYearIds;
			...
			return Terms.rawCollection().bulkWrite(fixtureTerms)
		}).then((terms) => {
			let termIds = _.values(terms.insertedIds)
			createdTermIds = termIds;
			...
			return Weeks.rawCollection().bulkWrite(fixtureWeeks)
		}).then((weeks) => {
			let weekIds = _.values(weeks.insertedIds)
			createdWeekIds = weekIds;
			return Resources.rawCollection().bulkWrite(fixtureResources)
		}).then((resources) => {
			let resourceIds = _.values(resources.insertedIds)
			createdResourceIds = resourceIds;

			sourceSchoolWork.forEach(schoolWork => {
				if (schoolWork.insertOne.document.resourceTitles) {
					schoolWork.insertOne.document.resources = Resources.find({groupId: groupId, title: {$in: schoolWork.insertOne.document.resourceTitles}}).map(resource => resource._id);
				} else {
					schoolWork.insertOne.document.resources = [];
				}
				delete schoolWork.insertOne.document.resourceTitles
			});

			createdSchoolYearIds.forEach(schoolYearId => {
				createdStudentIds.forEach(studentId => {
					sourceSchoolWork.forEach(schoolWork => {
						fixtureSchoolWork.push(
							{insertOne: {"document": {
								_id: Random.id(),
								name: schoolWork.insertOne.document.name,
								description: schoolWork.insertOne.document.description,
								resources: schoolWork.insertOne.document.resources,
								schoolYearId: schoolYearId,
								studentId: studentId,
								groupId: groupId, 
								userId: userId, 
			...
			return SchoolWork.rawCollection().bulkWrite(fixtureSchoolWork)
		}).then((schoolWork) => {
						...
						weekIds.forEach((weekId, weekIndex) => {
							schoolWorkIds.forEach((schoolWorkId, schoolWorkIndex) => {
								for (i = 0; i < timesPerWeek[schoolWorkIndex]; i++) {
									...
									let lessonProperties = {insertOne: {"document": {
										_id: Random.id(),
									...
									if (schoolYear.startYear === '2018' && term.order === 2) {
										if (lessonProperties.insertOne.document.order > 3.9 && lessonProperties.insertOne.document.order <= parseFloat('5.' + Math.floor(timesPerWeek[i] / 2).toString())) {
											lessonProperties.insertOne.document.completed = true
										} else {
											lessonProperties.insertOne.document.completed = false
										}

										if (lessonProperties.insertOne.document.order < 4) {
											lessonProperties.insertOne.document.completed = true
										}
									}
			...
			return Lessons.rawCollection().bulkWrite(fixtureLessons);
		}).then((lessons) => {
			let lessonIds = _.values(lessons.insertedIds)
