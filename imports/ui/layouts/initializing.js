...
Template.initializing.onRendered( function() {
	if (Session.get('initialized')) {
		if (Meteor.user().info.role === 'Observer') {
			FlowRouter.go('/tracking/students/view/' + Session.get('selectedStudentId') +'/'+ Session.get('selectedSchoolYearId') +'/'+ Session.get('selectedTermId') +'/'+ Session.get('selectedWeekId'));
